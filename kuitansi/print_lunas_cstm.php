...
$id = intval($_GET['id']);
$sql = "SELECT t.kode_transaksi, t.nama_customer, 
               t.cstm_produk, 
               b.cstm_bahan, 
               t.jumlah, t.diskon, t.harga, t.tax, t.total, t.subtotal,
               t.tanggal_transaksi, t.status_pembayaran, t.dp_amount, t.remaining_amount,
               t.status_pengiriman, t.pembayaran, t.pembelian, t.estimasi
        FROM transaksi t
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $mail->Body    = '
            ...
            <div><strong>Estimasi:</strong> ' . htmlspecialchars($data['estimasi']) . '</div>
            <table>
                <tr><th>Produk</th><th>Bahan</th><th>Jumlah</th></tr>
                <tr><td>' . htmlspecialchars($data['cstm_produk']) . '</td><td>' . htmlspecialchars($data['cstm_bahan']) . '</td><td>' . htmlspecialchars($data['jumlah']) . '</td></tr>
            </table>
            <table>
...
<div class="kuitansi">
    ...
    <table>
        ...
        <tr>
            <td><?= htmlspecialchars($data['cstm_produk']) ?></td>
            <td><?= htmlspecialchars($data['cstm_bahan']) ?></td>
...
<form method="POST" action="">
    ...
    <button type="submit"
    <?php if (!isset($_SESSION['level']) || $_SESSION['level'] !== 'Admin' && $_SESSION['level'] !== 'Kasir') echo 'disabled style="opacity: 0.6; cursor: not-allowed;" title="Hanya admin dan kasir yang bisa mengirimkan email ini"'; ?>>
    Kirim Kuitansi via Email</button>
</form>
